...
const criarTarefa = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: err.message + `Não foi possivel criar a tarefa` });
  }
};
...
const obterTodasTarefas = async (req, res) => {
  try {
    ...
    res
      .status(404)
      .json({ message: err.message + `Não foi possivel obter as tarefas` });
  }
};
...
const deletarTarefa = async (req, res) => {
  ...
  try {
    ...
    res
      .status(404)
      .json({ message: err.message + `Não foi possivel deletar a tarefa` });
  }
};
...
const editarTarefa = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: err.message + `Não foi possivel atualizar a tarefa` });
  }
};
